# qc_model.py
"""
QC model (cleaned port of your original inference.py)
- TinyMultiLabelMLP implemented in numpy
- Model wrapper: load / save / auto-train / predict multilabel QC notes
- Exposes: Model class, generateQcNotes(detectionData), trainModelFromFile(jsonPath)
"""
...
MODEL_PATH = PROJECT_ROOT / "models" / "solarQcModel.pkl"
MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)

AUTO_TRAIN_ON_START = True
EMB_DIM = 512  # default embedding dimension to pad/truncate to
...
class TinyMultiLabelMLP:
    def __init__(self, inputDim, hidden=128, outDim=25, seed=42):
        ...
        self.b2 = np.zeros(self.outDim, dtype=float)
    ...
    def fit(self, X, Y, lr=5e-3, epochs=300, batch=32, verbose=False):
        ...
        for epoch in range(epochs):
            ...
            if verbose and (epoch % 50 == 0 or epoch == epochs - 1):
                print(f"[QC-MLP] epoch {epoch+1}/{epochs} avgLoss={(epoch_loss / N):.6f}")

# ---------------------------
# Model Wrapper
# ---------------------------
class Model:
    def __init__(self, modelPath: Optional[str] = None, trainingJsonPath: Optional[str] = None, emb_dim: int = EMB_DIM):
        self.modelPath = Path(modelPath) if modelPath else MODEL_PATH
        self.trainingJsonPath = Path(trainingJsonPath) if trainingJsonPath else TRAINING_JSON_PATH
    ...
    def autoTrainIfNeeded(self):
        ...
        modelTs = self.modelPath.stat().st_mtime if self.modelPath.exists() else 0
